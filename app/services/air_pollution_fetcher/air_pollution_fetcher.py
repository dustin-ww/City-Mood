...
REDIS_LAST_FETCH_KEY = "air-pollution:last_fetch"
...
class AirPollutionFetcher(BaseFetcher):
    ...
    def send_daily_air_pollution(self, data: dict):
        ...
        producer = get_kafka_producer()
        event = {
            "fetch_timestamp": datetime.now(datetime.timezone.utc),
            "source": "open-meteo",
    ...
    def process_air_pollution(self):
        ...
        if last_fetch:
            ...
            if elapsed < fetch_interval:
                logger.info(
                    f"Skipping air pollution fetch. "
                    f"{elapsed/3600:.2f}h elapsed, "
                    f"required {fetch_interval/3600:.2f}h"
                )
                return
...
if __name__ == "__main__":
    fetcher = AirPollutionFetcher(
        wakeup_topic="fetch-air-pollution",
